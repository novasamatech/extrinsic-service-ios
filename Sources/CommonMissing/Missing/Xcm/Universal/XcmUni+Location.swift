...
extension XcmUni {
    public struct RelativeLocation: Equatable {
        let parents: UInt8
        let interior: XcmUni.Junctions

        public init(parents: UInt8, interior: XcmUni.Junctions) {
            self.parents = parents
            self.interior = interior
        }

        public init(parents: UInt8, items: [XcmUni.Junction]) {
            self.parents = parents
            interior = .init(items: items)
        }
    }

    public typealias AbsoluteLocation = XcmUni.Junctions
}

public extension XcmUni.RelativeLocation {
