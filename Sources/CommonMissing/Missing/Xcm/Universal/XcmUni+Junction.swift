...
extension XcmUni {
    public enum NetworkId: Equatable {
        case any
        case other(RawName, RawValue)
    }

    public struct AccountId32: Equatable {
        let network: NetworkId
        let accountId: AccountId
    }

    public struct AccountId20: Equatable {
        let network: NetworkId
        let accountId: AccountId
    }

    public enum Junction: Equatable {
    ...
    public struct Junctions: Equatable {
        let items: [Junction]

        public init(items: [Junction]) {
            self.items = items
        }
    }

    public struct GeneralKeyValue: Equatable {
...
public extension XcmUni.Junctions {
...
extension XcmUni.NetworkId: XcmUniCodable {
    public init(from decoder: any Decoder, configuration _: Xcm.Version) throws {
...
extension XcmUni.GeneralKeyValue: XcmUniCodable {
    public enum PostV3CodingKeys: String, CodingKey {
        case length
        case data
    }
